...
/** npm signal: package exists (0-12) + publish recency (0-18). */
export function npmScore(record) {
  ...
  if (sincePublish === null) {
    ...
    points += 18; notes.push("published within 90 days");
  } else if (sincePublish < 365) {
    points += 12; notes.push("published within a year");
  } else {
    points += 6; notes.push("last publish over a year ago");
  }
