...
export async function probeNpm(pkg, { fetchImpl = fetch }) {
  ...
  try {
    ...
    const latestManifest = typeof latest === "string" ? (versions[latest] ?? null) : null;
    let weeklyDownloads = null;
    try {
      const dl = await fetchImpl(NPM_DOWNLOADS + "/" + encodeURIComponent(pkg), {
        headers: { "user-agent": UA },
        signal: AbortSignal.timeout(10000),
      });
      if (dl.ok) weeklyDownloads = (await dl.json()).downloads ?? null;
    } catch { /* downloads unavailable — keep null */ }
    return {
      exists: true,
      name: pkg,
      version: latest ?? null,
      ...
      weeklyDownloads,
      dshBundle: !!(latestManifest && latestManifest.dsh && latestManifest.dsh.bundle),
    };
  } catch {
    ...
    for (let i = 0; i < candidates.length; i += NPM_CONCURRENCY) {
      ...
      batch.forEach((c, j) => {
        ...
        if (probe.exists) {
          ...
          if (rec) rec.npm = { name: c.pkg, version: probe.version, publishedAt: probe.publishedAt, created: probe.created, exists: true, weeklyDownloads: probe.weeklyDownloads, dshBundle: probe.dshBundle };
        }
      });
...
export function buildReport(record) {
  ...
  const dl = typeof record.npm?.weeklyDownloads === "number" ? record.npm.weeklyDownloads : null;
  lines.push("npm=" + (record.npm?.exists ? record.npm.name + "@" + (record.npm.version ?? "?") : "none") + (dl === null ? "" : " weeklyDownloads=" + dl) + " curated=" + (record.curated ? "yes" : "no") + " category=" + (record.category ?? "-"));
  if (score) {
  ...
  if (record.structure && record.structure.checkedAt) {
    lines.push("structure: patch=" + (record.structure.hasPatch ? "yes" : "no") + " bundle=" + (record.structure.hasBundle ? "yes" : "no") + " entry=" + (record.structure.hasEntry ? "yes" : "no") + (record.structure.dshEvidence ? "" : " — 疑似蹭标签（非 DSH 插件结构）"));
  }
  if (record.security) {
