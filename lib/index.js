...
function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      try {
        ...
        const date = String(summary.startedAt || new Date().toISOString()).slice(0, 10);
        const entries = records
          .filter((r) => typeof r?.score?.total === "number")
          .map((r) => ({ repo: r.repo, score: r.score.total, grade: r.score.grade ?? null }));
        const res = appendScoreSnapshots({ history: storage.loadScores(), entries, date });
        if (res.changed) storage.saveScores(res.history);
      } catch { /* score history must never break a sync */ }
  ...
  ctx.tools.register(defineTool({
    name: "audit_history",
    description: "Score history for the local plugin catalog: per-repo score timeline with trend direction (mode 'timeline', requires repo), or the biggest gainers/losers across all tracked repos over the last N samples (mode 'movers'). Snapshots are recorded by audit_sync.",
    parameters: {
      mode: {
        type: "string",
        enum: ["timeline", "movers"],
        description: "Report kind (default movers).",
      },
      repo: { type: "string", description: "owner/name to report a timeline for (required for mode=timeline)." },
      window: { type: "integer", description: "Movers: compare the last N samples (default 3)." },
      limit: { type: "integer", description: "Movers: rows per side (default 10, max 50)." },
    },
    output: {
      schema: {
        type: "object",
        ...
        properties: {
          ...
          gainers: { type: "array", items: { type: "object", additionalProperties: false } },
          losers: { type: "array", items: { type: "object", additionalProperties: false } },
    ...
    execute: (args) => {
      const mode = args.mode ?? "movers";
      const history = storage.loadScores();
      if (mode === "timeline") {
        const repo = args.repo;
        if (!repo) throw new Error("audit_history: repo is required for mode=timeline");
        const timeline = scoreTimeline({ history, repo });
        ...
        return {
          ...
          text: renderTimeline({ repo, timeline, trend }),
        };
      }
      const result = movers({
        history,
      ...
      return {
        ...
        text: renderMovers(result),
      };
    },
    presentCall: (args) => ({ card: "generic", title: "Audit history: " + (args.mode ?? "movers"), kind: "other", rawInput: args }),
  }));
